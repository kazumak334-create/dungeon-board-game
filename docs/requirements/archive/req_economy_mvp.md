...
| プレイヤー資源ゾーン | row 0-3 | 資源タイル（WOOD/STONE/WHEAT）が配置される |
...
- 各サイド保証数：WOOD×4, STONE×4, WHEAT×2
...
- §3.3 のロジックを継承（WOOD×4, STONE×4, WHEAT×2 per side, シード固定でシャッフル）
...
- **デフォルト値（プレイヤー）:** WOOD=1, STONE=1, WHEAT=0, ROLE_BUILD=0, ROLE_TRADE=0
...
```
[Wood ][Stone][Wheat][Build][Trade]
  1      1      0      1      0    ← 各役割の人数（直接 +/- ボタンで増減）
```
...
| ボタン名 | WOOD | STONE | WHEAT | 戦略意図 |
|---------|------|-------|-------|---------|
| 🌾 初心者 | 30 | 20 | 50 | 小麦優先→農村建設→安定収入 |
| ⚔️ 突特化 | 70 | 20 | 10 | 兵舎連打→突ユニット量産 |
| 🛡️ 守特化 | 30 | 60 | 10 | 要塞優先→守ユニットで前線維持 |
...
- 敵の初期資源: Wood=0, Stone=0, Wheat=10（プレイヤーと同じ初期小麦）
...
- WHEAT は後続ハーベスターが追加されたとき動的に調整することを想定（将来）
...
- プリセットボタン3種：初心者（小麦優先）、突特化、守特化
- ボタン1クリックで全スライダーを一括変更
- 初心者プリセット例：WHEAT=50, WOOD=30, STONE=20
- 突特化例：WOOD=70, STONE=20, WHEAT=10
- 守特化例：STONE=60, WOOD=30, WHEAT=10
