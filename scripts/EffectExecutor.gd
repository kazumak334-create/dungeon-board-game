...
func execute(effect_id: String, params: Dictionary, context: Dictionary) -> void:
	...
	match merged["type"]:
		...
		"spell_target_damage":     _actions.do_spell_target_damage(merged, ctx)
		...
		"curse_highest":            _actions.do_curse_highest(merged, ctx)
		"poison_add_periodic":     _actions.do_poison_add_periodic(merged, ctx)
		"debuff_conditional":      _actions.do_debuff_conditional(merged, ctx)
		"atk_boost_periodic":      _actions.do_atk_boost_periodic(merged, ctx)
		"disable_effects":         _actions.do_disable_effects(merged, ctx)
		# ---- バッチB: 未実装スキル対応 ----
		"debuff_apply_periodic":   _actions.do_debuff_apply_periodic(merged, ctx)
		"temp_buff_race":           _actions.do_temp_buff_race(merged, ctx)
		"grant_skill_flag":         _actions.do_grant_skill_flag(merged, ctx)
		"atk_by_buff_stacks":       _actions.do_atk_by_buff_stacks(merged, ctx)
		"curse_random":             _actions.do_curse_random(merged, ctx)
		"curse_front_random":       _actions.do_curse_front_random(merged, ctx)
		"curse_adjacent":           _actions.do_curse_adjacent(merged, ctx)
		"curse_amplify_all":        _actions.do_curse_amplify_all(merged, ctx)
		"rush_damage":              _actions.do_rush_damage(merged, ctx)
		"on_ally_death_trigger":    _actions.do_on_ally_death_trigger(merged, ctx)
		_:
