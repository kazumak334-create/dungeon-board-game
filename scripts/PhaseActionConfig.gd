...
# --- NEXT_EI 回復 ---
const HEAL_BASE_COST: int = 30       # 初回回復コスト
const HEAL_COST_INCREMENT: int = 20  # 回数ごとの追加コスト
const HEAL_RATIO: float = 0.30       # 回復割合（max_hpの30%）
...
static func get_heal_cost(use_count: int) -> int:
	return HEAL_BASE_COST + use_count * HEAL_COST_INCREMENT
