...
## ⚠️ セッション管理ルール（最優先）
- モデル: 必ず claude-sonnet-4-5 を使用（.claude/settings.jsonで強制固定済み）
- /clear タイミング: コミット完了後・タスク切り替え時・1時間経過時
- Agent起動時: MEMORY_SHARED.md のみ必須ロード。役割別ファイルは必要な時だけ読む
- Opus使用禁止: 複雑な設計判断が必要な場合のみCEOが1回だけ使用可

## ⚠️ memoryファイル管理ルール
- feedbackは発生後24時間以内にCLAUDE.mdに統合して削除する
- memory/フォルダのファイル総数が15本を超えたら即整理する
...
- Agent追加後の初回セッションでccusageを確認し、想定外の消費がないか確認する

## 開発プロセス・Agent運用ドキュメントの保存先

開発プロセス・Agent運用・組織設計に関するドキュメントは以下のディレクトリに保存する：

```
C:\Users\kazum\.claude\projects\C--Users-kazum-dungeon-board-game\docs\meta\
```

**対象例:**
- Agent構成・品質改善レポート
- 業務プロセス改善案
- Claude Code運用最適化プロンプト
- 会議運用ルール・テンプレート
- 差し戻しパターン分析

**docs/と区別する理由:**
- `docs/`: ゲーム開発本体（roadmap, world_concept, card_database等）
- `docs/meta/`（上記パス）: メタ情報（Agent運用、開発プロセス自体の改善）

新しいAgent運用案・プロセス改善案を作成した場合は必ずこの`docs/meta/`に保存し、CLAUDE.md側に要点だけ反映する。
...
| Agent | 役割 | 担当範囲 |
|---|---|---|
| ceo | 判断・振り分け・品質ゲート | ユーザー指示の解釈、Agent振り分け、差し戻しゲート、未決定事項の判断 |
| planning | 企画・仕様設計・世界観設計 | ゲームデザイン、カード企画、バランス、ストーリー、ナラティブ |
| marketing | 市場調査・競合分析・ペルソナ代弁 | 競合分析、差別化戦略、ペルソナインタビュー |
...
| pmo | 進捗管理・ロードマップ維持 | roadmap.md更新、backlog.md維持・週次棚卸し、朝報告作成、ブロッカー検出、CHANGELOG |
| pr | X・note投稿生成 | 発信コンテンツ作成 |

全Agent：Sonnet統一

### CEO の最重要責務: 5階層の整理（大ゴール → Phase毎ゴール → 論点 → 仮説 → タスク）

全ての連携パターンの起点で、CEOは必ず以下の5階層を整理する。これがないとAgentは方向性を失い、確定仕様を勝手に変更したり的外れな提案をする。

```
大ゴール → Phase毎ゴール → 論点 → 仮説 → タスク（仮説検証 → 検証結果 → 要件定義 → 実装）
```

**階層定義:**

- **大ゴール**: ゲーム全体の最終目標（docs/roadmap.md Phase 7完了条件）
- **Phase毎ゴール**: 現Phaseの完了条件（CLAUDE.md「現在のフェーズ」セクション）
- **論点**: 意思決定における重要な検討事項（何を決める必要があるか）
- **仮説**: その論点に対する現時点でのアイデア・解決方法
- **制約**: 変えてはいけないもの（既存確定仕様・ユーザー決定済み事項）
- **タスク**: 仮説検証+要件定義+実装の一連作業（下記4段階）

**タスクの4段階定義:**

タスクは「コードを書く」だけではない。以下の4段階を順に踏む：

1. **仮説検証**: 会議・調査・プロトタイプ・ペルソナレビュー等で仮説の妥当性を確かめる
2. **検証結果**: 何が分かったか・何が変わったかを明文化する
3. **要件定義**: 検証結果を踏まえた実装仕様（影響範囲・データ構造・UI・テスト観点）
4. **実装**: 実コードへの落とし込み + テスト追加 + checker検証

**Agentプロンプトのルール:**

CEOが全Agentに依頼する際、プロンプトには必ず以下5点を明記する：
...
制約: [変えてはいけないこと]
ペルソナ検証観点: [誰の反応を確認するか]
期待アウトプット: [選択肢2案 or 単一推奨案]
```

特に制約は既存仕様を列挙する形で明確化する（例: 「装備スロット=頭/胴/足/アクセ×3、変更不可」）。

**会議指示テンプレートなしの会議起動は禁止。**CEOがAgent起動前に上記7項目を必ず明記する。
...
4. **指摘分析会議（ペルソナ指摘がある場合は必須）**: ペルソナ指摘を構造分析し、対処案を検証する
   - **CEO**: 指摘を整理し、planning + marketing に分析を依頼（並列起動）
   - **planning**: 指摘の抽象化（個別指摘の裏にある共通の設計上の問題は何か？）+ 対処案の設計
   - **marketing**: 指摘の分類（構造的課題/提示順序/媒体/個人の好み）+ 対処案をペルソナに再インタビュー
   - **CEO**: 両者の分析結果を突合。分類が一致しない場合は再議論。一致したら対処案を確定
   - 安直な対処（1行追加等）で済ませない。根本原因に対処する
5. **CEO**: 合格した結果のみ統合。矛盾点を解消し、2-3案に絞る
6. **ユーザー**: 最終確認（方向性の分岐点のみ上げる）

**ルール:**
- 途中経過を逐一ユーザーに確認しない（CEOが判断）
- 批判的観点を必ず含める（「この案の弱点は？」「競合にすでにある？」）
- 参考作品が指定された場合は必ずWebSearchでリサーチしてから議論
- **テクノバブル禁止**: SF用語を使う前に「小学生に説明できる因果関係」を先に書く
- **指摘の安直対処禁止**: ペルソナ指摘に対して「1行追加」等の場当たり対処をしない。構造分析→根本対処→再検証のフローを必ず通す
...
| 名前 | 効果 | 発動位置 |
|------|------|---------|
| 狙撃 | 後列から敵最後列を優先攻撃、命中時スキル発動なし | 後列のみ |
...
| 合成結果 | 段数制限 | 合成場所 | UI表示 |
|---------|:---:|---------|---------|
| 消費アイテム | 2段まで | どこでも（DeckPrep持ち物タブ） | 解説レーン（直接合成のみ） |
| 中間素材 | 1段 | どこでも | 解説レーン |
| 装備 | 制限なし | 持ち物タブ | 持ち物タブ合成UI |
| カード | 制限なし | 持ち物タブ | カード詳細に派生ツリー表示 |

- カード合成はドラクエモンスター配合ツリー方式。深い階層を持てる
- カード強化は独立システムにしない。合成ツリーの枝として表現する
- カード+カード合成はバランス次第で保留（圧縮が同時に起きるため）
- 消費アイテム合成は簡易調合（現地でできる）。2段制限でUI複雑化を防ぐ
- 装備合成は持ち物タブで実行可能
...
| タブ | 内容 |
|------|------|
| 配置 | 自陣3×3+敵陣3×3の盤面UI。カード配置設定。宝石スロット（将来） |
| 持ち物 | マス目式インベントリ。上2行=装備スロット、以降=消費アイテム（左上優先）+素材（スタック表示） |
| スキル | スキルツリー |

- 装備着脱: 持ち物タブのマス目で操作
- 消費アイテム使用: 持ち物タブで選択→解説レーンに[使う]ボタン
- 消費アイテム合成: 素材選択→解説レーンに合成先表示→[作る]ボタン
- 装備合成: 持ち物タブで実行可能
- カード合成: 持ち物タブで実行可能
- 宝石: ドロップ専用（合成不可）。配置タブで使用

**マップノード種別:**
戦闘 / エリート / 素材採集 / ショップ / イベント / ボス
...
| 値 | 意味 | 例 |
|---|---|---|
| `"permanent"` | 通常カード（デフォルト） | 初期デッキ、報酬カード、呪いカード |
| `"battle"` | バトル終了後に自動消滅 | 異常状態カード、バトル中追加カード |

- デッキ除去の可否は`persistence`とは別軸。`removable`フィールド（デフォルトtrue）で制御
- `removable: false` はスキルで付与（例：特定の呪いスキル）。シャッフルカードもこれで統一

- `is_consumable`は「使用時に消滅」、`persistence`は「バトル/ラン終了時の存続」。直交する別軸
- バトル終了後のデッキクリーンアップ: `persistence == "battle"` のカードを `selected_deck` から除去
- **バトル中にデッキ追加されるカードのデフォルト:** `persistence = "battle"`（合成副産物、異常状態カード注入等）
- **例外:** スキルで`persistence_override: "permanent"`を持つカードは残せる（将来実装）
- 呪いカードは`persistence = "run"`で除去困難
...
**コミット自動トリガー（以下の全条件を満たした時にCEOが自動実行）:**
- テスト全パス（1件でもfailならコミット禁止）
- **新機能/修正に対するテストが追加されている**（テスト件数が増えていなければコミット禁止）
- 既知のバグが残っていない（ユーザー報告の未修正バグがあればコミット禁止）
- 機能の区切りが明確（「〇〇を実装した」と1文で説明できる粒度）
- 5ファイル以上の変更が溜まった時、または大機能完了時

**コミットしない条件（絶対）:**
- テストが1件でもfail
- **新機能を追加したのにテストが増えていない**
- ユーザー報告のバグが未修正
- 構文エラーがある（Godotで起動確認していない）
- 実装途中で中途半端な状態

**pushトリガー（以下の全条件を満たした時に自動実行）:**
- コミットが3つ以上溜まった時、または大機能完了時
- 全テストパス確認済み
- ユーザーに「pushする？」と聞かず自動実行（ただし結果は報告する）
