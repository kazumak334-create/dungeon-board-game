...
**本体能力システム:**
- プレイヤー本体が固有能力を持つ（盤面ユニットとは別レイヤー）
- 装備スロット（アーティファクトとは別）で本体能力を拡張
- 本体能力はバトル中常時発動し、デッキ構築・盤面効果・マナ回復等に影響
- データ構造: GameSession（Phase 3）に`player_abilities: Array`と`equipment: Array`を追加予定

**アーティファクトシステム:**
- 2タイプ：永久効果型（デッキ外・常時発動）＋ 盤面出現型（盤面配置・アクティブスキルのみ）
...
**R10. replace_all使用後はインデント検証を必ず行う**
